 ...
 * 1) Должна быть возможность оставить сообщение от гостя (с вводом имени)
 *          fixed
 * 2) Мелочь, но все-таки: юзеров хранить в файле users.txt
 *          fixed
 * 3) При удалении последнего сообщения на последней странице было бы неплохо перекидывать на "новую последнюю"
 *          fixed
 */
session_start();

//количество сообщений на странице
define('MESSAGE_ON_PAGE', 3);
...
define('ADMINS_GROUP', array('admin'));
...
do{
    ...
    if (isset($_POST['send'])) {
        ...
        //никнейм
        $nickName = $_POST['nick_name'] ?? false;

        //по умолчанию логин
        $name = $_SESSION['login'];

        //если введенные данные корреектны и такого пользователя нет,
        //то имя которое будет записано в guestbook задано корректно
        if ($nickName && correctInput($nickName)) {
            $name = array_key_exists($nickName, $usersArr) ? $name : $nickName;
        }

        //информация о сообщении
        $newMessageArr = array("$name", "$date", "$message", "$address");
...
)
{
    ...
    $tool = '<div class="tool">
                ...
                ><span>'.$__address.'</span>'.$buttonRemove.'
            </div>';
 ...
 * @return string
 */
function getMessCreateForGuest()
{
    //зона ввода сообщения
    $textArea = '<textarea rows="3" cols="50" autofocus name="message"></textarea>';

    //кнопка отправить и кнопка авторизации
    $send = '<input type="submit" name="send" value="Send">';
    $auth = '<input type="submit" name="authPage"
                                     value="authorization" class="buttonRed">';

    //ввод имени гостя
    $inputName = '<input type="text" name="nick_name" autocomplete="off"
                               placeholder="your nickname" class="nick_name">';
...
)
{
    ...
    $countMess = count($__usersData);

    //количество сообщений не включая текущую страницу
    $countPrevMess = $__messageOnPage * ($__numberPage - 1);

    //проверка на пустую страницу
    if ($countMess !== 0 && $countPrevMess === $countMess) {
        $_SESSION['numPage'] = $__numberPage - 1;
        return getMainPage(
            $__usersData, $__login, $__numberPage - 1,
            $__messageOnPage, $__refPagesAround
        );
    }
    ...
    //количество страниц
    $countPages  = intval($countMess / $__messageOnPage);
    $countPages += is_int($countMess / $__messageOnPage) ? 0 : 1;
    ...
    $body .= $__login === GUEST
        ? $adv. getMessCreateForGuest()
        : getMessageCreationBlock($__login);
    ...
    foreach (array_reverse($__usersData) as $userData) {
        ...
        //гость, увы, не может удалять сообщения свои
        if ($__login === GUEST) {
            $buttDel = false;
        }
